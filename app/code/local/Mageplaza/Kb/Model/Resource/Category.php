 ...
 * @author
 */
class Mageplaza_Kb_Model_Resource_Category extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    {
        $this->_init('mageplaza_kb/category', 'entity_id');
    }
    ...
    public function lookupStoreIds($categoryId)
    {
        ...
        $select  = $adapter->select()
            ->from($this->getTable('mageplaza_kb/category_store'), 'store_id')
     ...
     * @return Mageplaza_Kb_Model_Resource_Category
     * @author
     */
     ...
     * @param string $field
     * @param mixed $value
     * @param Mageplaza_Kb_Model_Category $object
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                array('kb_category_store' => $this->getTable('mageplaza_kb/category_store')),
                $this->getMainTable() . '.entity_id = kb_category_store.category_id',
                array()
            )
            ->where('kb_category_store.store_id IN (?)', $storeIds)
            ->order('kb_category_store.store_id DESC')
            ->limit(1);
        }
        return $select;
    }
    ...
    {
        if (!$this->_tree) {
            $this->_tree = Mage::getResourceModel('mageplaza_kb/category_tree')->load();
        }
        return $this->_tree;
    }
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        ...
        $table  = $this->getTable('mageplaza_kb/category_store');
     ...
     * @author
     */
    public function getParentCategorys($category)
    {
        $pathIds = array_reverse(explode('/', $category->getPath()));
        $categorys = Mage::getResourceModel('mageplaza_kb/category_collection')
     ...
     * @author
     */
    public function getChildrenCategorys($category)
    {
        $collection = $category->getCollection();
        $collection
            ->addIdFilter($category->getChildCategorys())
     ...
     * @return Varien_Data_Tree_Node_Collection|Mageplaza_Kb_Model_Resource_Category_Collection
     * @author
     */
    ...
    {
        $tree = Mage::getResourceModel('mageplaza_kb/category_tree');
    ...
    {
        return ($categoryId == Mage::helper('mageplaza_kb/category')->getRootCategoryId());
    }
     ...
     * @access public
     * @param Mageplaza_Kb_Model_Category $category
     * @param Mageplaza_Kb_Model_Category $newParent
     * @param null|int $afterCategoryId
     * @return Mageplaza_Kb_Model_Resource_Category
     * @author
     */
    public function changeParent(
        Mageplaza_Kb_Model_Category $category,
        Mageplaza_Kb_Model_Category $newParent,
    ...
    protected function _initCheckUrlKeySelect($urlKey, $store)
    {
        $select = $this->_getReadAdapter()->select()
            ...
            ->join(
                array('es' => $this->getTable('mageplaza_kb/category_store')),
