 ...
 * @author
 */
class Mageplaza_Kb_Model_Resource_Tag extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    {
        $this->_init('mageplaza_kb/tag', 'entity_id');
    }
    ...
    public function lookupStoreIds($tagId)
    {
        ...
        $select  = $adapter->select()
            ->from($this->getTable('mageplaza_kb/tag_store'), 'store_id')
     ...
     * @return Mageplaza_Kb_Model_Resource_Tag
     * @author
     */
     ...
     * @param string $field
     * @param mixed $value
     * @param Mageplaza_Kb_Model_Tag $object
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                array('kb_tag_store' => $this->getTable('mageplaza_kb/tag_store')),
                $this->getMainTable() . '.entity_id = kb_tag_store.tag_id',
                array()
            )
            ->where('kb_tag_store.store_id IN (?)', $storeIds)
            ->order('kb_tag_store.store_id DESC')
            ->limit(1);
        }
        return $select;
    }
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        ...
        $table  = $this->getTable('mageplaza_kb/tag_store');
    ...
    protected function _initCheckUrlKeySelect($urlKey, $store)
    {
        $select = $this->_getReadAdapter()->select()
            ...
            ->join(
                array('es' => $this->getTable('mageplaza_kb/tag_store')),
