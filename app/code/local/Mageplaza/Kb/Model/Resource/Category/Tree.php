 ...
 * @author
 */
class Mageplaza_Kb_Model_Resource_Category_Tree extends Varien_Data_Tree_Dbp
{
     ...
     *
     * @var Mageplaza_Kb_Model_Resource_Category_Collection
     */
    ...
    public function __construct()
    {
        ...
        parent::__construct(
            $resource->getConnection('mageplaza_kb_write'),
            $resource->getTableName('mageplaza_kb/category'),
            array(
     ...
     *
     * @access public
     * @param Mageplaza_Kb_Model_Resource_Category_Collection $collection
    ...
    {
        Mage::getResourceSingleton('mageplaza_kb/category')
    ...
    {
        Mage::app()->cleanCache(array(Mageplaza_Kb_Model_Category::CACHE_TAG));
        return $this;
    }
    ...
    protected function _createCollectionDataSelect($sorted = true)
    {
        ...
        $categorysTable = Mage::getSingleton('core/resource')
            ->getTableName('mageplaza_kb/category');
        $subConcat = $this->_conn->getConcatSql(array('main_table.path', $this->_conn->quote('/%')));
        $subSelect = $this->_conn->select()
            ->from(array('see' => $categorysTable), null)
    ...
    protected function _getInactiveItemIds($collection)
    {
        ...
        $table = Mage::getSingleton('core/resource')->getTable('mageplaza_kb/category');
        $bind = array(
            'cond' => 0,
        );
        $select = $this->_conn->select()
